...
const textResult = (value: unknown, title = 'BOT_404') => formatApiResult(value, title)
...
export async function handleApiCommand(sock: any, jid: string, command: Exclude<Command, null>) {
  ...
  if (name === 'tts') {
    ...
    const maybeIndex = /^\d+$/.test(a[a.length - 1]) ? Number(a.pop()) : 1
    if (maybeIndex < 1) return sock.sendMessage(jid, { text: 'voiceIndex must be a positive number from `.voices`.' })
    const out = await api.tts(a.join(' '), maybeIndex); return sock.sendMessage(jid, { audio: out.data, mimetype: 'audio/mpeg', ptt: false })
  }
  if (name === 'dl') {
  ...
  if (name === 'wiki') { const missing = need(a, '.wiki <article>'); if (missing) return sock.sendMessage(jid, { text: missing }); const out = await api.wikipediaPdf(a.join('_')); return sock.sendMessage(jid, { document: out.data, mimetype: out.contentType, fileName: 'wikipedia.pdf' }) }
